...
public interface InvoiceRepo extends JpaRepository<InvoiceEntity, String> {
	...
	@Query("SELECT invoice FROM InvoiceEntity invoice WHERE invoice.bookingStatus = 'Approved'")
	List<InvoiceEntity> findAllByBookingStatusApproved();
	
	@Query("SELECT invoice FROM InvoiceEntity invoice WHERE invoice.bookingStatus = 'Rejected'")
	List<InvoiceEntity> findAllByBookingStatusRejected();
